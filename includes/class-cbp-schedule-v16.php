...
/**
 * Raw-PDF Mass-intention fallback.
 *
 * The review source-lines list intentionally used array_unique(), which can
 * collapse repeated date headings. A real bulletin can have two Masses on the
 * same day (for example, an 11:00 AM funeral and the normal 5:00 PM Saturday
 * Mass). Once the second identical date heading is removed, later intention
 * repair passes cannot reliably separate those two Masses.
 *
 * This final pass reads the private preview PDF again, preserves duplicate
 * lines, rebuilds the dedicated "This week's Mass schedule" into per-Mass
 * chunks, and ONLY fills blank/suspicious Mass intentions. Existing good
 * intentions are never replaced.
 */
final class CBP_Schedule_V16
{
    ...
    {
        add_action('shutdown', array($this, 'postprocess_review'), 120);
    }
    ...
    public function postprocess_review()
    {
        ...
        try {
            ...
            // This is a best-effort repair pass. Never block the normal review
            // workflow if a PDF cannot be reparsed here.
            return;
        }
    ...
    private function raw_lines($text)
    {
        ...
        foreach ($parts as $line) {
            ...
            if ($line !== '') {
                // Deliberately do NOT de-duplicate. Repeated headings can mark
                // distinct Masses on the same date.
                $lines[] = $line;
            }
        }
        return $lines;
    ...
    private function mass_candidates(array $lines, $bulletin_date)
    {
        ...
        $chunks = array();
        $current = array();
        $limit = min(count($lines), $start + 120);
        ...
        foreach ($chunks as $chunk) {
            $candidate = $this->parse_chunk($chunk, $bulletin_date);
            if ($candidate !== null) {
                $result[] = $candidate;
            }
        }
    ...
    private function parse_chunk(array $chunk, $bulletin_date)
    {
