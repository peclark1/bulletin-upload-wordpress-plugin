...
/**
 * Final conservative Mass-intention fallback.
 *
 * Some PDFs split the date, time, parish, and intention for a single Mass
 * across several text lines. V14 repairs most of those cases, but a second
 * Saturday Mass on the same date can still be missed. This pass rebuilds the
 * dedicated weekly Mass schedule into per-Mass chunks and ONLY fills a blank
 * or obviously bad intention. Existing good intentions are never replaced.
 */
final class CBP_Schedule_V15
{
    ...
    {
        add_action('shutdown', array($this, 'postprocess_review'), 100);
    }
    ...
    public function postprocess_review()
    {
        ...
        $candidates = $this->mass_candidates($this->source_lines($review), $bulletin_date);
