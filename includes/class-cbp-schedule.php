...
final class CBP_Schedule
{
    ...
    private function diagnostic()
    {
        if (! extension_loaded('zlib')) {
            return new WP_Error('cbp_schedule_zlib', __('Automatic schedule extraction needs the PHP zlib extension, which is not available on this server. No website content has been changed.', 'church-bulletin-publisher'));
        }
        if (! extension_loaded('iconv')) {
            return new WP_Error('cbp_schedule_iconv', __('Automatic schedule extraction needs the PHP iconv extension, which is not available on this server. No website content has been changed.', 'church-bulletin-publisher'));
        }
        ...
        foreach ($lines as $index => $line) {
            ...
            if (strpos($lower, 'reconciliation') !== false || strpos($lower, 'confession') !== false) {
                if ($candidates['reconciliation'] === '') {
                    $candidates['reconciliation'] = $this->compact_schedule_line($line, $times);
                    $sources[] = $context;
                }
                continue;
            }
            if (strpos($lower, 'adoration') !== false) {
                if ($candidates['adoration'] === '') {
                    $candidates['adoration'] = $this->compact_schedule_line($line, $times);
                    $sources[] = $context;
                }
                continue;
            }
